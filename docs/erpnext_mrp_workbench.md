...
-   **Saved Filter Presets**: The **Filters ▾** button (next to Clear Filters) lets you save, load, and delete named snapshots of the current filter state. A preset captures the active text filters (include and exclude), number filters, the "Only show suggested" toggle, the active sort, and the selected column field. To save the current state, click **Filters ▾ → Save current filters…**, enter a name, and click **Save**. Saved presets appear in the dropdown; click a preset name to restore it instantly. To delete a preset, click the **✕** button on the right side of its row in the dropdown. When the current filters exactly match a saved preset, the button turns blue and displays the preset name. Presets are stored in the browser's `localStorage` and persist across sessions. Sharing a link: when a preset is active, its name is appended to the URL as `?preset=<name>` — opening that URL in the same browser automatically applies the preset.
