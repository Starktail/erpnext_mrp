...
Cypress.Commands.add("create_records", (doc) => {
	return cy.call("frappe.tests.ui_test_helpers.create_if_not_exists", { doc: JSON.stringify(doc) }).then((r) => r.message);
});
...
Cypress.Commands.add("click_listview_row_item_with_text", (text) => {
	cy.get(".list-row > .level-left > .list-subject > .level-item > .ellipsis").contains(text).first().click({ force: true });
});
