...
function stripNumberGroups(v, numberFormat) {
	...
	// strip groups (,)
	var groupRegex = new RegExp(info.groupSep === '.' ? '\\.' : info.groupSep, 'g')
...
export function formatNumber(v, format, decimals) {
	if (!format) {
		...
		if (decimals == null) decimals = cint(window.sysdefaults?.float_precision || 3)
	}
...
function roundNumber(num, precision, roundingMethod) {
	roundingMethod = roundingMethod || window.sysdefaults?.rounding_method || "Banker's Rounding (legacy)"
